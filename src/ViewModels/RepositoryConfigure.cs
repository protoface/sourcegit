...
namespace SourceGit.ViewModels {
	public class RepositoryConfigure : Popup {
		...
		public RepositoryConfigure(Repository repo) {
			...
			if (_cached.ContainsKey("user.name"))
				UserName = _cached["user.name"];
			if (_cached.ContainsKey("user.email"))
				UserEmail = _cached["user.email"];
			if (_cached.ContainsKey("commit.gpgsign"))
				GPGSigningEnabled = _cached["commit.gpgsign"] == "true";
			if (_cached.ContainsKey("user.signingkey"))
				GPGUserSigningKey = _cached["user.signingkey"];
			if (_cached.ContainsKey("http.proxy"))
				HttpProxy = _cached["user.signingkey"];
